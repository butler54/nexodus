Feature: CA API
  ...
  Scenario: Sign a CSR
    ...
    And the response should match json:
      """
      {"error":"cannot determine certificate url"}
      """
    ...
    Then "${cert.URIs.0 | string}" should match "https://api.try.nexodus.127.0.0.1.nip.io/api/sites/${site_id}"

    # the CA cert is per VPC..
    Then "${ca.Subject.CommonName}" should match "https://api.try.nexodus.127.0.0.1.nip.io/api/vpcs/${user_id}"
