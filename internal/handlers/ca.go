...
func (api *API) SignCSR(c *gin.Context) {
	...
	if tokenClaims != nil {
		...
		case "device-token":
			...
			certURI, err = url.Parse(fmt.Sprintf("%s/api/sites/%s", api.URL, siteId))
			if err != nil {
				c.JSON(http.StatusInternalServerError, models.NewApiError(err))
				return
			}
		}
	}
	...
	// this CA only enforces the URIs in the CSR for now
	template.URIs = append(template.URIs, certURI)
	...
	err = api.transaction(c, func(tx *gorm.DB) error {
		...
		if site.Vpc.CaCertificate == "" {
			cert, key, err := api.CreateVPCCertKeyPair(site.Vpc)
			if err != nil {
				return err
			}

			site.Vpc.CaCertificate = cert
			site.Vpc.CaKey = key
			if res := tx.Select("CaCertificate", "CaKey").
				Where("ca_certificate is NULL").
	...
	vpcCaKeyPair, err := ParseCertificateKeyPair([]byte(site.Vpc.CaCertificate), []byte(site.Vpc.CaKey))
	if err != nil {
		api.SendInternalServerError(c, err)
		return
	}
